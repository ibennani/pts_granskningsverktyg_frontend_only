// js/components/ConfirmDeleteCriterionViewComponent.js

export const ConfirmDeleteCriterionViewComponent = (function () {
    'use-strict';
    ...
    function handle_confirm_delete() {
        const { reqId, checkId, pcId } = params_ref;
    ...
    function render() {
        ...
        const check = current_state?.ruleFileContent?.requirements[reqId]?.checks.find(c => c.id === checkId);
        const criterion = check?.passCriteria.find(pc => pc.id === pcId);
        ...
        plate_element.appendChild(Helpers_create_element('h1', { text_content: t('confirm_delete_criterion_title') }));
        
        const warning_box = Helpers_create_element('div', { class_name: 'warning-box' });
        warning_box.innerHTML = `<p>${t('confirm_delete_criterion_intro')}</p><blockquote>${Helpers_escape_html(criterion.requirement)}</blockquote>`;
