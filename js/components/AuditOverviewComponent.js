...
export const AuditOverviewComponent = (function () {
    ...
    let Translation_t;
    let Helpers_create_element, Helpers_get_icon_svg, Helpers_format_iso_to_local_datetime, Helpers_escape_html, Helpers_get_current_iso_datetime_utc, Helpers_add_protocol_if_missing;
    ...
    function assign_globals() {
        ...
        } // else console.warn("AuditOverview: ExportLogic module not found on window. Exports will not work."); // Export är optionellt tills det behövs
    }
    ...
    function handle_lock_audit() {
        ...
        if (current_audit && current_audit.auditStatus === 'in_progress') {
            ...
            NotificationComponent_show_global_message(Translation_t('audit_locked_successfully'), 'success');
            render(); 
        }
    }
    function handle_unlock_audit() {
        ...
        if (current_audit && current_audit.auditStatus === 'locked') {
            ...
            State_setCurrentAudit(current_audit);
            NotificationComponent_show_global_message(Translation_t('audit_unlocked_successfully'), 'success');
            render(); 
        }
    }
    function handle_save_audit_to_file() {
        const current_audit = State_getCurrentAudit();
        if (!current_audit) { NotificationComponent_show_global_message(Translation_t('no_audit_data_to_save'), "error"); return; }
        try {
            ...
            if (link.download !== undefined) {
                ...
                NotificationComponent_show_global_message(Translation_t('audit_saved_as_file', {filename: filename}), "success");
            } else { NotificationComponent_show_global_message(Translation_t('browser_does_not_support_download'), "warning");}
        } catch (error) { console.error("Kunde inte serialisera eller spara granskningen:", error); NotificationComponent_show_global_message(Translation_t('error_saving_audit'), "error");}
    }
    ...
    function create_info_item(label_key, value, is_html = false) {
        ...
        const strong = Helpers_create_element('strong', { text_content: Translation_t(label_key) + ':' });
    ...
    async function init_sub_components() {
        ...
        } else { console.error("AuditOverview: SampleListComponent är inte korrekt initierad."); }
    }
    ...
    async function init(_app_container, _router) {
        ...
        if (window.Helpers && typeof window.Helpers.load_css === 'function') {
            try { await window.Helpers.load_css(CSS_PATH); } 
    ...
    function render() {
        if (!app_container_ref || !Helpers_create_element || !Translation_t || !State_getCurrentAudit) {
            ...
            if(app_container_ref) app_container_ref.innerHTML = `<p>${Translation_t ? Translation_t('error_render_overview') : "Could not render overview."}</p>`;
            return;
        }
        ...
        if (!current_audit || !current_audit.ruleFileContent) {
            if(NotificationComponent_show_global_message) NotificationComponent_show_global_message(Translation_t("error_no_active_audit"), "error");
            const go_to_upload_btn = Helpers_create_element('button', { text_content: Translation_t('start_new_audit'), class_name: ['button', 'button-primary'], event_listeners: { click: () => router_ref('upload') } });
        ...
        if (global_message_element_ref) {
            ...
            // Låt meddelandet vara kvar, t.ex. "Granskning påbörjad".
            // Det kan rensas av en efterföljande åtgärd om nödvändigt.
        }

        plate_element.appendChild(Helpers_create_element('h1', { text_content: Translation_t('audit_overview_title') }));

        // --- Sektion 1: Granskningsinformation ---
        const section1 = Helpers_create_element('section', { class_name: 'audit-overview-section' });
        section1.appendChild(Helpers_create_element('h2', { text_content: Translation_t('audit_info_title') }));
        ...
        info_grid.appendChild(create_info_item('status', Translation_t(`audit_status_${current_audit.auditStatus}`)));
        ...
        if (md.internalComment) {
            const comment_header = Helpers_create_element('h3', { text_content: Translation_t('internal_comment'), style: 'font-size: 1rem; margin-top: 1rem; font-weight: 500;' });
        ...
        section2.appendChild(Helpers_create_element('h2', { text_content: Translation_t('sample_list_title') }));
        ...
        } else { section2.appendChild(Helpers_create_element('p', {text_content: "Fel: Stickprovslistan kunde inte laddas."}));}
        ...
        section3.appendChild(Helpers_create_element('h2', { text_content: Translation_t('audit_actions_title') }));
        const actions_div = Helpers_create_element('div', { class_name: 'audit-overview-actions' });

        const save_button = Helpers_create_element('button', { class_name: ['button', 'button-default'], html_content: Helpers_get_icon_svg('save', ['currentColor'], 18) + `<span>${Translation_t('save_audit_to_file')}</span>` });
        ...
        if (current_audit.auditStatus === 'in_progress') {
            const lock_btn = Helpers_create_element('button', { class_name: ['button', 'button-warning'], html_content: Helpers_get_icon_svg('lock_audit', ['currentColor'], 18) + `<span>${Translation_t('lock_audit')}</span>` });
            lock_btn.addEventListener('click', handle_lock_audit);
            actions_div.appendChild(lock_btn);
        }

        if (current_audit.auditStatus === 'locked') {
            const unlock_btn = Helpers_create_element('button', { class_name: ['button', 'button-secondary'], html_content: Helpers_get_icon_svg('unlock_audit', ['currentColor'], 18) + `<span>${Translation_t('unlock_audit')}</span>` });
            unlock_btn.addEventListener('click', handle_unlock_audit);
            actions_div.appendChild(unlock_btn);

            if(ExportLogic_export_to_csv) {
                const csv_btn = Helpers_create_element('button', { class_name: ['button', 'button-info'], html_content: Helpers_get_icon_svg('export', ['currentColor'], 18) + `<span>${Translation_t('export_to_csv')}</span>` });
                csv_btn.addEventListener('click', handle_export_csv);
                actions_div.appendChild(csv_btn);
            }
            if(ExportLogic_export_to_excel) {
                const excel_btn = Helpers_create_element('button', { class_name: ['button', 'button-info'], html_content: Helpers_get_icon_svg('export', ['currentColor'], 18) + `<span>${Translation_t('export_to_excel')}</span>` });
