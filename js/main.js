...
(function () {
    ...
    let current_view_name_rendered = null; // Håller reda på vad som faktiskt renderats
    ...
    // Funktion som komponenter anropar för att byta vy OCH uppdatera URL-hashen
    function navigate_and_set_hash(target_view_name, target_params = {}) {
        ...
        if (window.location.hash === new_hash) {
            // Om hashen redan är korrekt, tvinga en om-rendering av vyn.
            // Detta kan hända om man t.ex. klickar på en länk som leder till samma sida man redan är på.
            ...
            // Ändra hashen, vilket kommer att trigga handle_hash_change som i sin tur anropar render_view.
            window.location.hash = new_hash;
        }
    }

    // Denna funktion renderar bara vyn, ändrar inte hashen.
    async function render_view(view_name_to_render, params_to_render = {}) {
        if (!window.Translation || typeof window.Translation.t !== 'function' || !window.Helpers || !window.Helpers.escape_html) {
            ...
            if(app_container) app_container.innerHTML = "<p>System error rendering view. Check console.</p>";
            return;
        }
        const { t } = Translation; 
    ...
    function handle_hash_change() { 
        ...
        // Rendera vyn om hashen (vynamn eller parametrar) faktiskt har ändrats från vad som senast renderades
    ...
    function on_language_changed_event() {
        ...
        if (current_view_name_rendered) { // Använd den senast renderade vyn
            ...
            // Om ingen vy har renderats än (t.ex. vid initial laddning om språk ställs in mycket tidigt),
            // förlita dig på den initiala handle_hash_change.
    ...
    async function init_app() { 
        ...
        handle_hash_change(); // Anropa en gång för att ladda initial vy baserat på aktuell hash
        
        update_app_chrome_texts(); // Uppdatera efter att allt är klart
