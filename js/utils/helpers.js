...
(function () { // IIFE start
    ...
    function get_icon_svg(icon_name, colors = [], size = 24) {
        ...
        const second_color = colors[1] || fill_color; // För ikoner med två färger
        let svg_path = '';
        ...
        const base_paths = {
            ...
            'arrow_forward_alt': `<path d="M12 4l-1.41 1.41L16.17 11H4v2h12.17l-5.58 5.59L12 20l8-8-8-8z"/>`, // Samma som arrow_forward, byt om du har specifik
            ...
            'lock_audit': `<path d="M18 8h-1V6c0-2.76-2.24-5-5-5S7 3.24 7 6v2H6c-1.1 0-2 .9-2 2v10c0 1.1.9 2 2 2h12c1.1 0 2-.9 2-2V10c0-1.1-.9-2-2-2zm-6 9c-1.1 0-2-.9-2-2s.9-2 2-2 2 .9 2 2-.9 2-2 2zm3.1-9H8.9V6c0-1.71 1.39-3.1 3.1-3.1 1.71 0 3.1 1.39 3.1 3.1v2z"/>`, // Lock icon
            'unlock_audit': `<path d="M18 8h-1V6c0-2.76-2.24-5-5-5S7 3.24 7 6h2c0-1.71 1.39-3.1 3.1-3.1S13.81 4.29 15 6v2H6c-1.1 0-2 .9-2 2v10c0 1.1.9 2 2 2h12c1.1 0 2-.9 2-2V10c0-1.1-.9-2-2-2zm-6 9c-1.1 0-2-.9-2-2s.9-2 2-2 2 .9 2 2-.9 2-2 2z"/>`, // Lock_open icon
            'export': `<path d="M19 9h-4V3H9v6H5l7 7 7-7zM5 18v2h14v-2H5v-2z"/>`, // File_download icon
            'light_mode': `<path d="M20 8.69V4h-4.69L12 .69 8.69 4H4v4.69L.69 12 4 15.31V20h4.69L12 23.31 15.31 20H20v-4.69L23.31 12 20 8.69zM12 18c-3.31 0-6-2.69-6-6s2.69-6 6-6 6 2.69 6 6-2.69 6-6 6zm0-10c-2.21 0-4 1.79-4 4s1.79 4 4 4 4-1.79 4-4-1.79-4-4-4z"/>`, // Light_mode (sun)
            'dark_mode': `<path d="M10 2c-1.82 0-3.53.5-5 1.35C7.99 5.08 10 8.3 10 12s-2.01 6.92-5 8.65C6.47 21.5 8.18 22 10 22c5.52 0 10-4.48 10-10S15.52 2 10 2z"/>`,          // Dark_mode (moon)
            'upload_file': `<path d="M9 16h6v-6h4l-7-7-7 7h4v6zm-4 2h14v2H5v-2z"/>`, // Detta är en uppåtpil med en bas
            'start_new': `<path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm-2 15l-5-5 1.41-1.41L10 14.17l7.59-7.59L19 8l-9 9z"/>`, // play_circle_filled eller add_circle
            'load_existing': `<path d="M19 12v7H5v-7H3v7c0 1.1.9 2 2 2h14c1.1 0 2-.9 2-2v-7h-2zm-6 .67l2.59-2.58L17 11.5l-5 5-5-5 1.41-1.41L11 12.67V3h2v9.67z"/>`, // Folder_open eller system_update_alt
            'visit_url': `<path d="M19 19H5V5h7V3H5c-1.11 0-2 .9-2 2v14c0 1.1.89 2 2 2h14c1.1 0 2-.9 2-2v-7h-2v7zM14 3v2h3.59l-9.83 9.83 1.41 1.41L19 6.41V10h2V3h-7z"/>`, // Open_in_new
            'audit_sample': `<path d="M14 2H6c-1.1 0-1.99.9-1.99 2L4 20c0 1.1.89 2 1.99 2H18c1.1 0 2-.9 2-2V8l-6-6zm2 16H8v-2h8v2zm0-4H8v-2h8v2zm-3-5V3.5L18.5 9H13z"/>`, // Description / Find_in_page
            ...
            'check_circle_green_yellow': `<path fill="${fill_color}" d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm-2 15l-5-5 1.41-1.41L10 14.17l7.59-7.59L19 8l-9 9z"/>` // Förenklad, behöver anpassas för två färger.
        };

        svg_path = base_paths[icon_name];

        if (!svg_path) {
            console.warn(`[Helpers.get_icon_svg] Unknown icon "${icon_name}". Using default placeholder.`);
            svg_path = `<circle cx="12" cy="12" r="10" stroke="${fill_color}" stroke-width="1.5" fill="none" /><text x="12" y="16" font-size="10" text-anchor="middle" fill="${fill_color}">?</text>`;
        }
